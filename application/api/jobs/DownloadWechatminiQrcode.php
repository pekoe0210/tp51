...
class DownloadWechatminiQrcode
{
    ...
    private function doJob($data)
    {
        ...
        if ($contents) {
            ...
            $qrcode_url = 'uploads/wechatmini/qrcode/' . $file_name;
            $this->mkdirs(dirname($qrcode_url));
            file_put_contents($qrcode_url, $contents);

            $wechatmini_model = new Wechatmini();
            $update = $wechatmini_model->where('id', '=', $data['wechatmini_id'])
                ->update(['qrcode_url' => $qrcode_url]);
